...
const UI = {
    ...
    createMessageHTML(msg, isGrouped = false) {
        ...
        return `<div class="${rowClass}" ${msgId}><div><div class="msg-bubble">${actionsBtn}<div class="msg-bubble-content">${this.escapeHtml(msg.body)}</div><div class="msg-bubble-footer"><span class="msg-bubble-time">${this.formatTime(msg.created_at)}</span>${editedTag}${isSent ? '<span class="msg-bubble-ticks"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"></polyline></svg></span>' : ''}</div></div>${actionsMenu}</div></div>`;
    },
